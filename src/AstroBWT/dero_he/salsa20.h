...
__device__ __forceinline__ uint32_t ROTATE(uint32_t a, uint32_t b)
{
...
__device__ __forceinline__ uint32_t XOR(uint32_t v, uint32_t w)    { return v ^ w; }
__device__ __forceinline__ uint32_t PLUS(uint32_t v, uint32_t w)   { return v + w; }

__global__ void __launch_bounds__(32) Salsa20_XORKeyStream(const uint32_t* keys, uint32_t* outputs)
{
	const uint32_t t = threadIdx.x;
	const uint32_t g = blockIdx.x;

	const uint32_t output_offset = g * 10240;
	
	const uint32_t* k = keys + g * 8;
	uint32_t* output = outputs + (output_offset + (t * 64)) / sizeof(uint32_t);
	...
	const uint32_t j0 = 0x61707865UL;
	const uint32_t j5 = 0x3320646EUL;
	const uint32_t j10 = 0x79622D32UL;
	const uint32_t j15 = 0x6B206574UL;
	const uint32_t j6 = 0;
	const uint32_t j7 = 0;
	const uint32_t j8 = 0;
	const uint32_t j9 = 0;

	for (uint32_t i = t * 64; i < output_size; i += 32 * 64)
	{
		...
		output += (32 * 64) / sizeof(uint32_t);
	}

	sync();

	// Put zeros after output's end
	if (t < 16)
	{
		uint32_t* p = outputs + (output_offset + output_size + 3) / sizeof(uint32_t);
		p[t] = 0;
	}
